...
@interface CJReportProgressView ()
@property (nonatomic,strong)UILabel * dataLabel;
@property (nonatomic,strong)UILabel * dataNameLabel;

@end

@implementation CJReportProgressView
...
+(instancetype)createProgressWithFrame:(CGRect)frame backColor:(UIColor *)backColor color:(UIColor *)color proportion:(CGFloat)proportion{
    CJReportProgressView * progressView = [[self alloc]initWithFrame:frame];
    if (!color) {
...
-(UILabel *)dataLabel{
    if (!_dataLabel) {
        ...
        _dataLabel.center                       = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2 - self.bounds.size.height/8+28);
        _dataLabel.font                         = [UIFont font_fontPingFangSC:36];

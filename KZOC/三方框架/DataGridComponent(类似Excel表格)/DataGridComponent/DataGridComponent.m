...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if([t tapCount] == 1){
		...
		int idx = [t locationInView:self].y / d.cellHeight; //根据点击的位置，判断点击在哪一行
...
- (id)initWithFrame:(CGRect)aRect data:(DataGridComponentDataSource*)aDataSource{
	...
	if(self != nil){
		...
		contentWidth = .0;
		cellHeight = 30.0;
		cellWidth = [[dataSource.columnWidth objectAtIndex:0] intValue]; //单元格默认宽度
		for(int i=1;i<[dataSource.columnWidth count];i++)
			contentWidth += [[dataSource.columnWidth objectAtIndex:i] intValue];  //每行内容的总宽度
		contentHeight = [dataSource.data count] * cellHeight;	//内容的总高度（不带标题）
...
-(void)layoutSubView:(CGRect)aRect{
	...
	//初始化各视图
	vTopLeft = [[UIView alloc] initWithFrame:CGRectMake(0, 0, cellWidth, cellHeight)];//UIView左上标题，一个单元格大小
    
	vLeft = [[DataGridScrollView alloc] initWithFrame:CGRectMake(0, cellHeight, aRect.size.width, aRect.size.height - cellHeight)];//下列视图 DataGridScrollView，标题以下视图大小
    
	vRight = [[DataGridScrollView alloc] initWithFrame:CGRectMake(cellWidth, 0, aRect.size.width - cellWidth, contentHeight)];// 除了第一列的视图大小
    
	vTopRight = [[UIView alloc] initWithFrame:CGRectMake(cellWidth, 0, aRect.size.width - cellWidth, cellHeight)];//标题栏，除了第一列剩下的（即右上）UIView
	
	vLeft.dataGridComponent = self;
	vRight.dataGridComponent = self;
	
	vLeft.opaque = YES;  //不透明的
	vRight.opaque = YES;
	vTopLeft.opaque = YES;
...
-(void)fillData{
	...
	for(int column = 0;column < [dataSource.titles count];column++){
		...
		UILabel *l = [[UILabel alloc] initWithFrame:CGRectMake(columnOffset, 0, columnWidth -1, cellHeight )];//添加label
...
//		[l release];
	...
	for(int i = 0;i<[dataSource.data count];i++){
		...
		for(int column=0;column<[rowData count];column++){
			...
			l.textAlignment = NSTextAlignmentCenter;
			l.tag = i * cellHeight + column + 1000;  //注意每个label的标识很重要
			if(i % 2 == 0)
				l.backgroundColor = [UIColor whiteColor];  //奇偶单元格的颜色是不一样的
